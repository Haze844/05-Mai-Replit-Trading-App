...
// Statistik-Berechnungen für die angezeigten Trades, unterteilt nach Benutzer
  ...
  const { data: adminTrades = [], isLoading: isLoadingAdmin, refetch: refetchAdmin } = useQuery<any[], Error>({
    ...
    queryFn: async () => {
      ...
      // Streng nach userId 1 (Jasper) filtern
      const jasperTrades = data.filter(trade => Number(trade.userId) === 1);
      console.log("Jasper Trades STRENG gefiltert:", jasperTrades.length);
      return jasperTrades;
    },
    refetchOnMount: true,
    staleTime: 0
  ...
  const { data: moTrades = [], isLoading: isLoadingMo, refetch: refetchMo } = useQuery<any[], Error>({
    ...
    queryFn: async () => {
      ...
      // Streng nach userId 2 (Mo) filtern
      const moTrades = data.filter(trade => Number(trade.userId) === 2);
      console.log("Mo Trades STRENG gefiltert:", moTrades.length);
      return moTrades;
    },
    refetchOnMount: true,
    staleTime: 0
